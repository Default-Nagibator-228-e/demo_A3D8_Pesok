package alternativa.proplib
{
   ...
   public class PropLibrary
   {
      ...
      public function PropLibrary(files:ByteArrayMap,gh:Config)
      {
         super();
		 loi = gh;
         if(files == null)
      ...
      private function parsePropMesh(propXml:XML) : PropMesh
      {
         ...
         var modelData:ByteArray = this.files.getValue(propXml.@file.toString());
         var textureFiles:Object = null;
		 loi.xml = XML((this.files.getValue(propXml.llp.@file.toString())).toString());
         if(propXml.texture.length() > 0)
         {
            textureFiles = {};
            for each(textureXml in propXml.texture)
            {
               textureFiles[XMLUtils.copyXMLString(textureXml.@name)] = textureXml.attribute("diffuse-map").toString();
            }
         }
